...
from sympy import *
var('a,b,wa,wm,wn,wb')
#rule = lambda f: wa*f(a) + wm*f(a+((b-a)/3.0))+ wn*f(a+(2./3.)*(b-a))+ wb*f(b)
# Note: it may be that floating point is screwing this up
rule = lambda f: wa*f(a) + wm*f(a+((b-a)/3))+ wn*f(a+2*(b-a)/3)+ wb*f(b)
...
b2 = lambda x: x**2
b3 = lambda x: x**3
# require that b0, b1, b2, b3 are integrated exactly
var('x')
...
R = solve((e0,e1,e2,e3),(wa,wm,wn,wb))
print R
v = rule(f)
s = Subs(v,(wa,wm,wn,wb),(R[wa],R[wm],R[wn],R[wb])).doit()
formula = factor(s)
